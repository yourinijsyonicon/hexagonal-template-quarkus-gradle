...
import be.yonicon.template.UpdateCustomerTestPresenter;
...
public class UpdateCustomerTest implements CustomerTraits {
    ...
    @Nested
    class TestWhen {
        private UpdateCustomerTestPresenter presenter;

        @BeforeEach
        void setup() {
            createCustomer(createDto("some commercial name", "some legal name", "some contact person", "some vat nr"));

            CustomerId existingCustomerId = findCustomerByVatNr("some vat nr")
                    .map(CustomerItem::getId)
                    .orElseThrow();

            presenter = updateCustomer(existingCustomerId, createDto(COMMERCIAL_NAME, LEGAL_NAME, CONTACT_PERSON, VAT_NUMBER));
        }

        @Test
        void itShouldBeUpdated() {
    ...
    class TestWhenTryingToUpdateWithNoExistingCustomerId {
        ...
        @BeforeEach
        void setup() {
            presenter = updateCustomer(CustomerId.newId(), createDto("not relevant", "not relevant", "not relevant", VAT_NUMBER));
        }

        @Test
        void itShouldNotBeCreated() {
            assertThat(getContext().listCustomer.getAllCustomers()).isEmpty();
        }
